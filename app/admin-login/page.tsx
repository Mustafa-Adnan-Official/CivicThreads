...
function AdminLoginForm() {
  ...
  // If user doc is already loaded (e.g. page refresh while logged in), redirect immediately
  useEffect(() => {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (!fbUser) { setError("Sign in failed."); return; }
      ...
      // Email verified — read the user doc ourselves for instant redirect
      // (don't wait for onAuthStateChanged to finish its async Firestore read)
      setRedirecting(true);
      if (db) {
        try {
          const snap = await getDoc(doc(db, "users", fbUser.uid));
          const role = snap.exists() ? (snap.data().role as string) : "RESIDENT";
          if (role === "CITY_ADMIN") { router.replace("/admin"); return; }
  ...
  async function handleResend() {
    setError("");
    try { await sendVerification(); setResendSent(true); }
    ...
    <div className="min-h-screen flex items-center justify-center bg-zinc-50 dark:bg-zinc-950 px-4">
      <div className="w-full max-w-sm">
        ...
        <form onSubmit={handleSubmit} className="bg-white dark:bg-zinc-900 rounded-lg border border-zinc-200 dark:border-zinc-800 p-6 shadow-sm">
          <h1 className="text-lg font-semibold text-zinc-900 dark:text-zinc-50 mb-1">Admin / Rep Sign In</h1>
          <p className="text-sm text-zinc-500 dark:text-zinc-400 mb-4">For city admins and ward representatives</p>
          ...
          <div className="space-y-4">
            <div>
              ...
              <input id="email" type="email" value={email} onChange={(e) => setEmail(e.target.value)} required
                className="w-full px-3 py-2 border border-zinc-300 dark:border-zinc-600 rounded-md bg-white dark:bg-zinc-800 text-zinc-900 dark:text-zinc-100 focus:outline-none focus:ring-2 focus:ring-zinc-400" placeholder="you@city.gov" />
            </div>
            <div>
            ...
            <div className="flex justify-end text-sm">
              <button type="button" onClick={handleForgotPassword} className="text-zinc-600 dark:text-zinc-400 hover:underline">Forgot password?</button>
            </div>
            {resetSent && <p className="text-sm text-emerald-600 dark:text-emerald-400">Password reset email sent.</p>}
          </div>
          {showUnverified && (
            <div className="mt-4 p-3 rounded-md bg-amber-50 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-800">
              <p className="text-sm text-amber-800 dark:text-amber-200 mb-2">Email not verified yet.</p>
              <button type="button" onClick={handleResend} disabled={resendSent}
        ...
        <div className="mt-4 text-center text-sm text-zinc-600 dark:text-zinc-400 space-y-1">
          <p>City Admin? <Link href="/admin-signup" className="font-medium text-zinc-900 dark:text-zinc-100 hover:underline">Sign up</Link></p>
          <p>Ward Rep? <Link href="/rep-signup" className="font-medium text-zinc-900 dark:text-zinc-100 hover:underline">Sign up</Link></p>
        </div>
